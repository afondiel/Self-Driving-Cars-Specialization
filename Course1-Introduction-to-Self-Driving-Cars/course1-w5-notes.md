...
- `Control Layer` : defines the setpoints, acceleration and deceleration to be tracked precisely. The main task that needs to  be perfomed by both Longitudinal and Lateral control is to `follow the plan`as well as minimizing the error btw the actual and reference path and speed
