...
- Wheel
  ...
  - $\displaystyle T_{whl}  = I_{w}\dot{\omega}_{w} + r_{eff} F_{x}$

- Transmission
  - $\displaystyle I_{t} \dot{\omega}_{t} = T_{t} - (GR)T_{whl}$
...
- Engine 
  - $\displaystyle I_{e} \dot{\omega}_{e} = T_{Eng} - T_{t}$
  
  - $\displaystyle I_{e} \dot{\omega}_{e} = T_{Eng} - I_{t} + I_{w}GR^2 \dot{\omega}_{e} + GRr_{eff} F_{x}$
...
- Combining with our engine dynamics model yields : 
  - $\displaystyle [I_{e} + I_{t} + I_{w}(GR^2) + m(GR^2)r^2_{eff}] \dot{\omega}_{e} = T_{Eng} - GRr_{eff} F_{load}$
